...
const userNumbersKm = parseFloat(prompt('Type How many kilometers do you want to travel?'));
const userAge = parseFloat(prompt('What s your age?'));
const ticket = 0.267113 * userNumbersKm;
console. log(userNumbersKm, userAge);
console. log(ticket);

if ( userAge < 21){
    const saleUnderage = ( ticket /100 )* 24.552
    const priceTicketOne = ticket - saleUnderage
    console. log(priceTicketOne)
} 
if (userAge > 63){
    const saleOver = ( ticket /100 )* 37.893
    const priceTicketTwo = ticket - saleOver
    console. log(priceTicketTwo)
}
